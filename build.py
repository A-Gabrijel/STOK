...
class rectok:
    ...
    def Openings(self, **kwargs):
        ...
        x = (self.outer_radius-self.containment_rest[0][3]/2)*m.cos((angle)*kwargs["index"])
        y = (self.outer_radius-self.containment_rest[0][3]/2)*m.sin((angle)*kwargs["index"])

        port_depth = self.SumOuterThickness_Full*(1/m.cos(m.radians(self.eq_ports_rest[kwargs["index"]][2])) + 1/m.cos(m.radians(self.eq_ports_rest[kwargs["index"]][2])))

        # Constructing objects to be used for later addition and subtraction
        if self.shape == "square":
            port_nohole = cq.Workplane("XY").\
                          box(port_depth*4, self.eq_ports_rest[kwargs["index"]][0], self.eq_ports_rest[kwargs["index"]][0])
        elif self.shape == "rectangle_z":
            port_nohole = cq.Workplane("XY").\
                          box(port_depth*4, self.eq_ports_rest[kwargs["index"]][0], self.longer_rect)
        elif self.shape == "rectangle_y":
            port_nohole = cq.Workplane("XY").\
                          box(port_depth*4, self.longer_rect, self.eq_ports_rest[kwargs["index"]][0])
        else:
        ...
        port_endcutDOWN = cq.Workplane("XY").\
                          ...
                          extrude(100).\
                          translate(Vector(0, 0, -self.containment_height/2 + self.SumLowerThickness_Full - 100))

        port_endcutUP = cq.Workplane("XY").\
                        circle(self.outer_radius).\
                        extrude(100).\
                        translate(Vector(0, 0, +self.containment_height/2 - self.SumUpperThickness_Full))
        ...
        port_cut = cq.Workplane("XY").\
                   ...
                   translate(Vector(0, 0, -(self.eq_ports_rest[kwargs["index"]][0])*100/2))

        allignXYport = self.SumOuterThickness_Full/m.cos(m.radians(self.eq_ports_rest[kwargs["index"]][3])) * m.sin(m.radians(self.eq_ports_rest[kwargs["index"]][3]))
        ...
        port = port.rotate((0, -self.eq_ports_rest[kwargs["index"]][0], 0), (0, self.eq_ports_rest[kwargs["index"]][0], 0), -self.eq_ports_rest[kwargs["index"]][2]).\
                    rotate((0, 0, -self.eq_ports_rest[kwargs["index"]][0]), (0, 0, self.eq_ports_rest[kwargs["index"]][0]), self.eq_ports_rest[kwargs["index"]][3])
        ...
        if self.eq_ports_rest[kwargs["index"]][2] < 0:
            port = port.translate(Vector(-self.outer_radius, -allignXYport-self.eq_ports_rest[kwargs["index"]][5], self.eq_ports_rest[kwargs["index"]][4])).\
                        ...
                        translate(Vector(self.outer_radius-self.containment_rest[0][3]/2, 0, -self.eq_ports_rest[kwargs["index"]][4]))
        else:
            port = port.translate(Vector(-self.outer_radius, -allignXYport-self.eq_ports_rest[kwargs["index"]][5], self.eq_ports_rest[kwargs["index"]][4])).\
        ...
        allignZ = self.SumOuterThickness_Full/m.cos(m.radians(self.eq_ports_rest[kwargs["index"]][2])) * m.sin(m.radians(self.eq_ports_rest[kwargs["index"]][2]))
        ...
        port = port.rotate((0, 0, self.eq_ports_rest[kwargs["index"]][0]),(0, 0, -self.eq_ports_rest[kwargs["index"]][0]), -180-degAngle).\
                    translate(Vector(x, y, self.eq_ports_rest[kwargs["index"]][4] - allignZ))
    ...
    def Port(self, **kwargs):
        ...
        allignZport = self.SumOuterThickness_Full/m.cos(m.radians(self.eq_ports_rest[kwargs["index"]][2])) * m.sin(m.radians(self.eq_ports_rest[kwargs["index"]][2]))

        port_depth = self.SumOuterThickness_Full/m.cos(m.radians(self.eq_ports_rest[kwargs["index"]][2]))*2 + self.SumOuterThickness_Full/4 if self.eq_ports_rest[kwargs["index"]][2] != 0 else self.SumOuterThickness_Full/m.cos(m.radians(self.eq_ports_rest[kwargs["index"]][3]))*2 + self.SumOuterThickness_Full/4


        # Constructing objects to be used for later addition and subtraction
        port_nohole = cq.Workplane("XY").box(port_depth, self.eq_ports_rest[kwargs["index"]][0], self.eq_ports_rest[kwargs["index"]][0])

        port_hole = cq.Workplane("XY").box(port_depth, self.eq_ports_rest[kwargs["index"]][1], self.eq_ports_rest[kwargs["index"]][1])
        ...
        # Here we cut the hole into the port
        port = port_nohole.cut(port_hole).translate(Vector(self.SumOuterThickness_Full,0,0))
        ...
        # Finally the persistent rotation of the final geometry is applied (so all the ports face center)
        port = port.rotate((0, 0, self.eq_ports_rest[kwargs["index"]][0]),(0, 0, -self.eq_ports_rest[kwargs["index"]][0]), -degAngle+180)
    ...
    def Limiter(self, **kwargs):
        ...
        x_s = (self.outer_radius+self.containment_rest[0][3]/2-self.SumOuterThickness_Full+self.limiter_thickness/2+self.firstwall_thickness/2)*m.cos(angle*kwargs["index"])
        y_s = (self.outer_radius+self.containment_rest[0][3]/2-self.SumOuterThickness_Full+self.limiter_thickness/2+self.firstwall_thickness/2)*m.sin(angle*kwargs["index"])

        x = (self.outer_radius+self.containment_rest[0][3]/2-self.SumOuterThickness_Full)*m.cos(angle*kwargs["index"])
        y = (self.outer_radius+self.containment_rest[0][3]/2-self.SumOuterThickness_Full)*m.sin(angle*kwargs["index"])


        allignZport = self.SumOuterThickness_Full/m.cos(m.radians(self.eq_ports_rest[kwargs["index"]][2])) * m.sin(m.radians(self.eq_ports_rest[kwargs["index"]][2]))

        # Constructing objects to be used for later addition and subtraction
        firstwall = cq.Workplane("XY").box(self.firstwall_thickness, self.eq_ports_rest[kwargs["index"]][1] - self.limiter_gap, self.eq_ports_rest[kwargs["index"]][1] - self.limiter_gap)

        firstwall_rear = cq.Workplane("XY").box(self.limiter_thickness, self.eq_ports_rest[kwargs["index"]][1] - self.limiter_gap, self.eq_ports_rest[kwargs["index"]][1] - self.limiter_gap)
        ...
        # Finally the persistent rotation of the final geometry is applied (so all the ports face center)
        firstwall = firstwall.rotate((0, 0, self.eq_ports_rest[kwargs["index"]][0]),(0, 0, -self.eq_ports_rest[kwargs["index"]][0]), -degAngle+180)
        firstwall_rear = firstwall_rear.rotate((0, 0, self.eq_ports_rest[kwargs["index"]][0]),(0, 0, -self.eq_ports_rest[kwargs["index"]][0]), -degAngle+180)
    ...
    def BoundingBox(self, **kwargs):
        ...
        bounding_box_outer = cq.Workplane("XY").box(self.limb_radius+self.outer_radius * 8 + self.bbox_thickness, self.limb_radius+self.outer_radius * 8 + self.bbox_thickness, self.solenoid_height * 4 + self.bbox_thickness)
        bounding_box_inner = cq.Workplane("XY").box(self.limb_radius+self.outer_radius * 8, self.limb_radius+self.outer_radius * 8, self.solenoid_height * 4)
    ...
    def Spheres(self, **kwargs):
        ...
        sphere_right = cq.Workplane("XY").sphere(self.sphere_radius).translate(Vector(0,self.sphere_radius+self.limbs_rest[kwargs["index"]][1]/2))
        sphere_left = cq.Workplane("XY").sphere(self.sphere_radius).translate(Vector(0,-self.sphere_radius-self.limbs_rest[kwargs["index"]][1]/2))
    ...
    def PlasmaSrc(self, **kwargs):
        ...
        cyl_inner_r = self.solenoid_radius + self.SumInnerThickness_Full + distance_from_wall
        cyl_height = self.containment_height - self.SumLowerThickness_Full - self.SumUpperThickness_Full - 2*distance_from_wall
        print(cyl_inner_r)
        print(cyl_outer_r)
        print(cyl_height)

        cyl = cq.Workplane("XY").cylinder(cyl_height, cyl_outer_r).cut(cq.Workplane("XY").cylinder(cyl_height, cyl_inner_r))
...
class GeometryInjector():
    ...
    def Body(self, **kwargs):
        ...
        line = "body {} {} {}\n".format(kwargs["body_name"], kwargs["body_name"], kwargs["material"])

        self.InjectLine(inject_string=line)

    def File(self, **kwargs):

        line = "file {} \"{}\" {} 0 0 0\n".format(kwargs["object_name"], kwargs["filepath"], kwargs["scale"])
